        ...
        <v-dialog v-if="dialog" v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition" @keydown.esc="dialog = false">
            <v-card>
                <v-toolbar dark color="primary">
                    ...
                    <v-toolbar-title>Nom de l'usuari</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-toolbar-items>
                ...
                <v-layout row wrap>
                     <v-flex xs12>
                        <user-name-manage :user="user" @saved="saved" @close="dialog=false"></user-name-manage>
                    </v-flex>
                </v-layout>
                </v-container>
            </v-card>
...
<script>
import UserNameManage from './UserNameManage'
export default {
  name: 'UserEditName',
  components: {
    'user-name-manage': UserNameManage
  },
  data () {
    return {
      dialog: false
