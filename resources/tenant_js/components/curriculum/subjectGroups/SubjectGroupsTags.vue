<template>
    <span>
        <v-chip small label :color="tag.color" text-color="white" v-for="(tag,key) in subjectGroupTags" :key="tag.id" close v-model="close[key]">
...
export default {
  ...
  watch: {
    subjectGroup: {
      handler: function (subjectGroup) {
        this.sync(subjectGroup.tags)
      },
      deep: true
    },
    close (close) {
      let tagObjToRemove = this.subjectGroupTags[close.indexOf(false)]
  ...
  methods: {
    pendingTagsToAssign () {
      const tagsIds = this.subjectGroupTags.map(tag => tag['id'])
    ...
    add () {
      this.adding = true
      window.axios.post('/api/v1/subjectGroups/' + this.subjectGroup.id + '/tags/' + this.newTag.id, {}).then(() => {
        this.$emit('refresh')
        this.adding = false
    ...
    remove (tag) {
      this.removing = true
      const url = '/api/v1/subjectGroups/' + this.subjectGroup.id + '/tags/' + tag.id
    ...
    syncClose () {
      this.close = []
      this.subjectGroupTags.forEach(tag => {
        this.close.push(true)
      })
    },
    sync (tags) {
      this.subjectGroupTags = this.subjectGroup.tags
      this.syncClose()
      this.pendingTags = this.pendingTagsToAssign()
    }
  },
  created () {
    this.sync(this.subjectGroup.tags)
  }
}
</script>
