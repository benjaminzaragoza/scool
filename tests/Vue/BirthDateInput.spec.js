import { shallow, mount, createLocalVue } from 'vue-test-utils'
import expect from 'expect'
import BirthDateInput from '../../resources/assets/tenant_js/components/ui/BirthDateInputComponent.vue'
import Vuetify from 'vuetify'
import VueLoader from 'vue'

import moxios from 'moxios'
// https://github.com/JeffreyWay/laravel-mix/issues/1748
window.Date = Date // temporary bug fix, should be removed after vue-test-utils fixes #936

describe('BirthDateInput', () => {
  let wrp

  beforeEach (() => {
    moxios.install(axios)

    const localVue = createLocalVue()
    localVue.use(Vuetify)
    localVue.use(Vuetify)

    wrp = mount(BirthDateInput, {
      localVue: localVue,
    })
  })

  afterEach (() => {
    moxios.uninstall(axios)
  })

  it('renders the correct title on the page', () => {
    let wrapper = shallow(BirthDateInput)

    expect(wrapper.html()).toContain('Todo List')
  })

  // it('shows the todos fetched from the api', (done) => {
  //   let wrapper = mount(TodoList)
  //
  //   moxios.stubRequest('/todos', {
  //     status: 200,
  //     response: [
  //       {
  //         id: 1,
  //         text: "Bingo",
  //         finished: false,
  //         created_at: "2018-01-10 00:00:00",
  //         updated_at: "2018-01-10 00:00:00",
  //       }
  //     ],
  //   })
  //
  //   moxios.wait(() => {
  //     expect(wrapper.html()).toContain('Bingo')
  //     done()
  //   })
  // })

  // it('shows the empty state if no todos are fetched from the api', (done) => {
  //   let wrapper = mount(TodoList)
  //
  //   moxios.stubRequest('/todos', {
  //     status: 200,
  //     response: [],
  //   })
  //
  //   moxios.wait(() => {
  //     expect(wrapper.html()).toContain('There are no todos')
  //     done()
  //   })
  // })
})